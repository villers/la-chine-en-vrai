...
// Function to fetch config from API at runtime
async function getFirebaseConfig() {
  ...
  // Fallback to process.env for server-side or development
  firebaseConfig = {
...
export async function initializeFirebase() {
  ...
  if (hasValidConfig) {
    ...
    console.error('Firebase configuration is invalid:', config);
  }

  return { app, db, auth, storage };
}

// Export Firebase services (they will be null until initialized)
